...
def download_fine_tuned(model_dir):
    if not (Path().absolute() / model_dir).exists():
        print("Downloading model ... ")
        http_response = urlopen("https://github.com/tinsirius/ECE4078_Practicals_2022/releases/download/1.0.6/fine-tuned.zip")
        zipfile = ZipFile(BytesIO(http_response.read()))
        zipfile.extractall(path=Path(model_dir).parents[0])
        print("Done!")